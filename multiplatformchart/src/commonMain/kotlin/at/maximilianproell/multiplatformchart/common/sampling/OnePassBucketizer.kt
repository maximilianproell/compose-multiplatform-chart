...
internal class OnePassBucketizer {
    companion object {
        fun bucketize(input: List<Point>, inputSize: Int, desiredBuckets: Int): List<Bucket> {
            val middleSize = inputSize - 2 // Minus first and last point
            ...
            var rest = input.subList(1, input.lastIndex)

            // Add middle buckets.
            // When inputSize is not a multiple of desiredBuckets, remaining elements are equally distributed
            ...
            buckets.add(Bucket.of(input.last()))
            return buckets
        }
    }
}